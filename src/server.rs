            ...
            let n: usize = 'client_read: {
                ...
                loop {
                    ...
                    if backend_gone {
                        ...
                        // session alive. With `tr_mode = none` outside a
                        // transaction the next query simply redials (as before).
                        // Otherwise the next request is handled as a
                        // not-delivered backend fault against this node: a
                        // transaction that was open died with the socket (it must
                        // NOT silently continue in autocommit on a fresh
                        // connection), and session state (SETs) is restored on
                        ...
                        if session.tr_mode != TrMode::None
                            || session
                                ...
                                .load(std::sync::atomic::Ordering::Relaxed)
                        {
                            tr.lost_backend = Some(node.to_string());
                        }
                        break 'client_read stream
                            .read_buf(&mut buffer)
                            .await
